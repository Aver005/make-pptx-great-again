...
page.on('request', req => {
  ...
  else { blocked.push(url); req.abort() }
})
...
const steps = []
const log = line => { steps.push(line); console.log('  ' + line) }
...
await page.waitForFunction(() => document.getElementById('summary').textContent.includes('10'), { timeout: 40000 })
...
const notes = await page.$$eval('.note', nodes => nodes.map(n => n.textContent.trim().slice(0, 70)))
...
console.log(`  сетевых запросов наружу: ${blocked.length}${blocked.length ? ' — ' + blocked.slice(0, 3).join(', ') : ' (ни одного)'}`)
console.log(`  ошибок в консоли: ${errors.length}${errors.length ? ' — ' + errors.slice(0, 3).join(' | ') : ''}`)
