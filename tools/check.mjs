...
const result = await page.evaluate(async source => {
  ...
  const { ir, warnings } = await window.MPGA.convert(source, {
    keepFrame: true,
    onFrame: frame => { window.__frame = frame },
  })
  ...
  return {
    ...
    stats: ir.slides.map(s => ({
      i: s.index + 1, boxes: s.boxes.length, texts: s.texts.length,
      images: s.images.length, missing: s.images.filter(im => im.missing).length,
...
console.log(`${name}: ${result.stats.length} слайдов, ${result.ms} мс, epx=${result.epx.toFixed(1)}, слайд ${result.slideW}×${result.slideH}`)
for (const s of result.stats) {
  console.log(`  слайд ${s.i}: фигур ${s.boxes}, текстов ${s.texts} (${s.runs} фрагментов), картинок ${s.images}${s.missing ? `, пустых ${s.missing}` : ''}`)
}
...
if (withRender) {
  ...
  const count = await page.evaluate(() => window.__frame.contentDocument.querySelectorAll('.mpga-slide-root').length)
  ...
  execFileSync('soffice', ['--headless', '--convert-to', 'pdf', '--outdir', workDir, pptxPath], { stdio: 'ignore' })
  execFileSync('pdftoppm', ['-png', '-r', '96', join(workDir, `${name}.pdf`), join(workDir, 'got')])
  ...
  for (let i = 1; i <= count; i++) {
    ...
    let raw = '0'
    try {
      raw = String(execFileSync('compare',
        ['-metric', 'AE', '-fuzz', '8%', join(workDir, `a-${n}.png`), join(workDir, `b-${n}.png`), norm],
