(() => {
  ...
  const SVG_PROPS = [
    ...
    'font-weight', 'font-style', 'text-anchor', 'letter-spacing', 'paint-order',
  ]
  ...
  function bakeDom(win, src, clone) {
    ...
    for (const [a, b] of pairs) {
      if (b.nodeName === 'SCRIPT' || b.nodeName === 'STYLE') { b.remove(); continue }
      const st = win.getComputedStyle(a)
      let css = ''
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        try {
          ...
          for (let i = 3; i < data.length; i += 4) { if (data[i] > 8) { empty = false; break } }
  ...
  async function captureSvg(win, el, rect, hideText, scale) {
    ...
    if (!clone.getAttribute('viewBox')) {
      ...
      if (box) clone.setAttribute('viewBox', `0 0 ${Math.max(1, box.width + box.x)} ${Math.max(1, box.height + box.y)}`)
    }
    ...
    for (const slide of ir.slides) {
      for (const cap of slide.captures) {
        ...
        try {
          ...
          else notes.push({ code: 'empty-raster', level: 'warn', slide: slide.index,
          ...
          notes.push({ code: 'raster-failed', level: 'warn', slide: slide.index,
  ...
  async function resolveImages(ir) {
    ...
    for (const slide of ir.slides) {
      for (const im of slide.images) {
        ...
        if (/^data:/i.test(im.src)) { im.data = im.src; continue }
        try {
          ...
          im.missing = true
          notes.push({ code: 'image-unreachable', level: 'warn', slide: slide.index,
            text: `Слайд ${slide.index + 1}: картинку по ссылке вставить не удалось, на её месте будет пустая рамка.`,
            fix: 'Не вставляй картинки по ссылке из интернета — браузер не даёт их прочитать. ' +
