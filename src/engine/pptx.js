    ...
    for (const slide of ir.slides) {
      ...
      for (const box of slide.boxes) {
        ...
        if (box.fill) {
          ...
          if (box.alpha != null && box.alpha < 1) opts.fill.transparency = Math.round((1 - box.alpha) * 100)
        } else {
          opts.fill = { color: 'FFFFFF', transparency: 100 }
        }
        if (box.stroke) opts.line = { color: box.stroke, width: Math.max(0.25, box.strokeW * 0.75) }
      ...
      for (const image of slide.images) {
        const opts = {
          x: toIn(image.x), y: toIn(image.y + dy),
          w: Math.max(0.01, toIn(image.w)), h: Math.max(0.01, toIn(image.h)),
        }
        if (image.rot) opts.rotate = image.rot
        if (image.data) s.addImage({ data: image.data, ...opts })
        else s.addShape('rect', { ...opts, fill: { color: 'F2F2F2' }, line: { color: 'BFBFBF', width: 1 } })
      }
      ...
      for (const text of slide.texts) {
        ...
        const opts = {
          x: toIn(left), y: toIn(text.y - 1 + dy),
          w: Math.max(0.05, toIn(text.w + padX)), h: Math.max(0.05, toIn(text.h + 2)),
          margin: 0, valign: 'middle', align, wrap,
