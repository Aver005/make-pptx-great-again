(() => {
  ...
  function paintIcons(win, notes = iconNotes) {
    ...
    for (const el of targets) {
      const name = (
        ...
        el.getAttribute("data-mpga-icon") ||
        ""
      )
        .trim()
        .toLowerCase();
      ...
      for (const attr of el.attributes) {
        ...
        if (attr.name === "class")
          ...
          svg.setAttribute("style", `${svg.getAttribute("style") || ""};${attr.value}`);
    ...
    if (missing.size) {
      ...
      notes.push({
        ...
        fix:
          `В презентации есть иконки, которых нет в наборе lucide: ${list}. ` +
  ...
  function stripExternal(doc, notes) {
    ...
    for (const el of doc.querySelectorAll(
      'link[rel~="stylesheet" i], link[rel="preload"][as="style"]',
    )) {
    ...
    if (cut.styles) {
      notes.push({
        ...
        fix:
          "Не подключай CSS-библиотеки по ссылке (Tailwind, Bootstrap и т. п.). " +
  ...
  function checkImages(doc, notes) {
    ...
    const bgUrls = [];
    for (const st of doc.querySelectorAll("style")) {
      for (const m of (st.textContent || "").matchAll(/url\(\s*["']?(https?:[^"')]+)/gi))
        bgUrls.push(m[1]);
    }
    if (remote.length || bgUrls.length) {
      notes.push({
        ...
        level: "warn",
        text:
          `Картинок по ссылке из интернета: ${remote.length + bgUrls.length}. ` +
          "Они могут не попасть в PPTX — их не разрешает читать браузер.",
        fix:
          'Не вставляй картинки по ссылке (<img src="https://...">). ' +
