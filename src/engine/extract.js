(() => {
  ...
  function transformInfo(value) {
    ...
    const simple = Math.abs(scaleX - 1) < 0.02 && Math.abs(scaleY - 1) < 0.02 && Math.abs(skew) < 0.5
  ...
  function extract(win, options = {}) {
    ...
    slideEls.forEach((slideEl, si) => {
      ...
      if (Math.abs(S.width - first.width) > 1) {
        warn('slide-size', `Слайд ${si + 1} другого размера, чем первый — он может обрезаться.`,
          'Сделай все слайды одного размера.', si)
      }
      ...
      for (const el of walkAll(slideEl)) {
        ...
        if (tag === 'svg') markRaster(el, 'svg')
        else if (['canvas', 'video', 'iframe', 'object', 'embed'].includes(tag)) markRaster(el, 'embed')
          ...
          else { markRaster(el, 'transform'); rotatedRoots.add(el) }
        }
      }
      ...
      if (rotatedRoots.size) {
        warn('rotated', `Повёрнутые блоки (${rotatedRoots.size} шт.) вставлены картинкой — текст в них не редактируется.`,
          'Не поворачивай блоки через transform: rotate — PowerPoint получает их картинкой.', si)
      }
      ...
      const unionRect = nodes => {
        ...
        for (const node of nodes) {
          let r
          if (node.nodeType === 3) { range.selectNode(node); r = range.getBoundingClientRect() }
      ...
      const pushBoxesFor = (el, st, r, geom) => {
        ...
        if (String(st.borderTopLeftRadius).includes('%')) radius = r.width * px(st.borderTopLeftRadius) / 100
        ...
        if (uniform) { box.stroke = sides[0].c.hex; box.strokeW = sides[0].w }
        boxes.push(box)
        ...
        if (!uniform) {
          for (const b of sides) {
            ...
            else if (b.side === 'bottom') { bar.y = geom.y + geom.h - b.w; bar.h = b.w }
            else if (b.side === 'left') bar.w = b.w
            else if (b.side === 'right') { bar.x = geom.x + geom.w - b.w; bar.w = b.w }
            boxes.push(bar)
          }
        }
      }
      ...
      const pushPseudo = (el, r, which) => {
        ...
        boxes.push({
          x: r.left - ox + x, y: r.top - oy + y, w, h,
          fill: fill.hex, alpha: fill.alpha, radius: px(p.borderTopLeftRadius),
        })
      }
      ...
      const collectRuns = el => {
        ...
        for (const node of el.childNodes) {
          ...
          if (node.querySelector('div,p,section,ul,ol,li,table,h1,h2,h3,h4,h5,h6,svg,img')) continue
      ...
      for (const el of walkAll(slideEl)) {
        ...
        const geom = spun
          ? {
              ...
              w: el.offsetWidth || r.width, h: el.offsetHeight || r.height, rot: spun,
            }
        ...
        if (tag === 'img') {
          ...
          images.push({
            id: `img-${si}-${captureSeq++}`, kind: 'img', src,
        ...
        texts.push({
          x: boxX - ox, y: ink.top - oy, w: boxW, h: ink.height,
          align: st.textAlign, lh, lines,
      ...
      function collectSvgText(svg, sr) {
        ...
        for (const node of svg.querySelectorAll('text')) {
          ...
          texts.push({
            x: tr.left - ox, y: tr.top - oy, w: tr.width, h: tr.height,
            align: anchor === 'middle' ? 'center' : anchor === 'end' ? 'right' : 'left',
            lh: size * 1.2, lines: 1, svgText: true,
            runs: [{
              ...
              spacing: 0, font: fontOf(tst.fontFamily),
            }],
          })
        }
      }
      ...
      if (Math.abs(aspect - SLIDE_H_EMU) > SLIDE_H_EMU * 0.02) {
        warn('aspect', `Слайд ${si + 1} не 16:9 — сверху и снизу останутся поля.`,
          'Делай слайды строго 16:9 (например 1160×652).', si)
      }
      ...
      slides.push({
        index: si, w: S.width, h: S.height,
        background: slideBg && slideBg.alpha > 0.9 ? slideBg.hex : null,
        boxes, texts, images, captures,
